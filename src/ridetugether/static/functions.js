...
var mycollege = new College("Taylor University", "236 W Reade Ave Upland,IN", 40.459252, -85.497146);
...
function initialize(mess) {
    ...
    if (request.status == 200) {
        ...
        var mymarker = new google.maps.Marker(new google.maps.LatLng(40.459406, -85.497152));
        mymarker.setOptions(icons.my);

        reqMarkerOptions = {
            icon: blueIcon
        };
        var initialLocation = new google.maps.LatLng(40.459406, -85.497152);
...
function makeRideTable() {
    ...
    table.style.minHeight = table.clientHeight + rides.length + "px";
}
...
function getNewRidePopupHTML(lat, lng, address3) {
    ...
    full += "<input onclick=\"newRidePopupHTMLPart2(" + lat + ", " + lng + ", '" + address3 + "', false);\" type=\"radio\" name=\"rideType\" value=\"0\" id=\"rideType\"" + "/>From " + mycollege.name + " to <br />";

    full += address3 + "<br />";

    full += "<input onclick=\"newRidePopupHTMLPart2(" + lat + ", " + lng + ", '" + address3 + "', true);\" type=\"radio\" name=\"rideType\" value=\"1\" id=\"rideType\"/>To " + mycollege.name + "</p></form>";
...
function getNewRideIsDriverHTML(lat, lng, address, to, contact) {
    ...
    full += "<form><p>"
    full += '<input onclick="newRidePopupHTMLPart2b(' + lat + "," + lng + ", '" + address + "'," + to + ", true);" + '" type="radio" name="driver" value="1" /> I will drive <br />';
    full += '<input onclick="newRidePopupHTMLPart2b(' + lat + "," + lng + ", '" + address + "'," + to + ", false);" + '" type="radio" name="driver" value="0" /> I am looking for a ride <br />';
    full += "</form></p>";
    full += "From:<br />";
    if (to) {
        full += "<b>"+address + "</b><br />";
    } else {
        full += "<b>"+mycollege.name + "</b><br />";
    }
    full += "To: <br />";
    if (to) {
        full += "<b>"+mycollege.name+"</b>";
    } else {
        full += "<b>"+address+"</b>";
    }
    return full;
...
function getNewRidePopupHTML2(lat, lng, address5, to, isDriver, contact) {
    contact = (typeof contact == 'undefined') ? '' : contact; // If contact is not defined, then let it be 563-555-1212
    ...
    line5 += "<div id=\"textToAll\">Please ensure that your address is as specific as possible<br/>From <input type=\"text\" id=\"textFrom\" name='textFrom' size=\"50\"";
    if (to == true) {
...
function getPopupWindowMessage2(rideNum, doOrPu, lat, lng, address8, contact) {
    ...
    var text = "Please ensure that your address is as specific as possible<br/>";
    if (doOrPu == 0) {
        text += "Drop Off At: ";
    } else {
        text += "Pick Up At: ";
...
function windowOpen(position1, content1) {
    ...
    boxText.innerHTML = content1;
    boxText.className = 'mapPopup';
    var myOptions = {
        position: position1,
        content: boxText,
    ...
    var window = new InfoBox(myOptions);
